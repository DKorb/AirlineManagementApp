...
const CreateNewTicket = () => {
    ...
    const handleAddNewTicket = async (e) => {
        ...
        try {
            const response = await fetch(`http://localhost:9090/api/v1/users/${formData.userId}/tickets`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                setSuccess('You created new ticket successfully!')
                setTimeout(() => {
                    history.push('/ticket')
    ...
    return (
        <div style={{ backgroundSize: 'cover', backgroundPosition: 'center', height: '80vh', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
            <div style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)', padding: '20px', borderRadius: '10px', boxShadow: '0 0 10px rgba(0, 0, 0, 0.5)', backdropFilter: 'blur(3px)', width: '600px' }}>
                <Form onSubmit={handleAddNewTicket} className="d-flex flex-column align-items-center">
                    ...
                    <h1 className="mb-4" style={{ color: 'white', fontSize: '50px', letterSpacing: '3px' }}>ADD NEW TICKET</h1>
                    <Form.Group className="mb-3" controlId="formGroupFlightId">
                        <Form.Control required style={{ width: '450px' }} type="text" name="flightId" value={formData.flightId} placeholder="Enter flight ID" onChange={handleChange} />
                    </Form.Group>
